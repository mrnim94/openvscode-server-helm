...
* **⚡ Sudo Access:** The environment comes with a non-root user (`antiantiops`) pre-configured with passwordless `sudo` privileges, allowing you to install system packages on the fly.
...
docker run -it -p 8000:8000 \
  -v $(pwd):/home/antiantiops/project \
  vscode-server:latest
```

* **Access the IDE:** Open your browser and navigate to `http://localhost:8000`.
* **Persist Data:** The command above maps your current directory to `/home/antiantiops/project`. Any changes made inside that folder will be saved to your host machine.
